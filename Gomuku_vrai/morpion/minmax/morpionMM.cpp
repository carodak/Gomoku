...
void jouerIA(sf::RenderWindow &window,int x,int y,int profondeur);
...
void Grille::jouerIA(sf::RenderWindow &window,int x,int y, int profondeur) {
       ...
       // IA_jouer(3);
        draw_croix(maxi-7,maxj-5,window);
        //Appel à l'IA qui doit jouer
			...
			//	IA_jouer(profondeur);
				//x=maxi; y=maxj;
             //   draw_rond(a.mouseButton.x,a.mouseButton.y,window);
      ...
      gagne=eval();
      if (gagne>0){
        cout<<"IA a gagné"<<endl;
      }
      if (gagne<0){
        cout<<"Joueur a gagné"<<endl;
      }
      if (gagne==0){
        cout<<"Match Nul"<<endl;
      }
